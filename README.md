...
| document | `j`/`k` block (each list item is a block); `c` comment on the block; `x` clear its annotations; `o` move the cursor with `hjkl` `w` `b` `0` `$`, then `v` select from there; `v` select with the same keys |
