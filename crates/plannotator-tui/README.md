...
| document | drag with the mouse, or `v` then `hjkl` / `w` `b` / `0` `$` to select; `Enter` confirms · `o` moves the cursor with those keys first, so `v` can start mid-block · `j`/`k` or click selects a block (each list item is one) · `c` comments on the block · `x` clears the block's annotations |
